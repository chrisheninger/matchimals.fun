...
**Localization.** User-facing strings never appear inline: they come from `src/locales/<locale>.ts` through `t()` / `caps()` / `playersLabel()` / `animalName()` in `src/i18n.ts` (English `en.ts` defines the `Translations` interface; every locale must implement it). Titles and buttons are stored in natural case and capitalised at render with `caps()`. `AnimalName` stays the English key everywhere — translate only when rendering. Text styles spread `...displayFont` (Dimbo, or the bold platform font for Japanese/Korean/Chinese) instead of naming `"Dimbo"`. After touching strings run `bun run check:locales` (keys, Dimbo glyph coverage, measured slot widths, store-metadata limits) and regenerate `store/translations-review.md` with `bun scripts/translations-review.mjs`; `bun run screenshots:locales` photographs every language on the simulators. The English output must stay byte-identical: the marketing site's gameplay recorder finds buttons by the English accessibility labels ("2 players", "Play as the Cat").
